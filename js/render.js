  ...
  function renderFicha(p) {
    ...
    if (descEl) {
      if (p.descripcion) {
        ...
        const wa = `https://wa.me/595981353110?text=${encodeURIComponent(`Hola, quisiera consultar por "${p.nombre}" (${p.sku}) de MoMar.`)}`;
    ...
    if (attrs) {
      ...
      if (entries.length === 0) {
        const wa = `https://wa.me/595981353110?text=${encodeURIComponent(`Hola, quisiera ver el detalle técnico de "${p.nombre}" (${p.sku}) de MoMar.`)}`;
    ...
    if (precio && !document.querySelector('.consulta-wa')) {
      ...
      precio.insertAdjacentHTML('afterend', `
        <a href="https://wa.me/595981353110?text=${msg}" target="_blank" rel="noopener" class="consulta-wa">
    ...
    if (!m) {
      ...
      m.innerHTML = `
        <div class="modal">
          ...
          <a href="https://wa.me/595981353110?text=${encodeURIComponent('Hola, quisiera pedir un medidor de talles gratis para mi anillo de MoMar.')}" target="_blank" class="btn btn--primary btn--block" style="margin-top: var(--space-3);">
